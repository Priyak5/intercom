...
3. **Live chat** — Open [/widget/test/](https://intercom-pk.up.railway.app/widget/test/) in a new tab (or `demo/index.html` served from any local origin). Append `?key=<your-workspace-public_key>` to the URL — find the key via `/api/auth/me`[https://intercom-pk.up.railway.app/api/auth/me](link) after signup, or `Workspace.objects.first().public_key` in `railway run python manage.py shell`. Send a message from the widget → it appears in the dashboard within a second; agent reply appears back in the widget instantly. Typing indicators, presence, and read receipts all live.
